...
//EDGE: adds reeded or fluted edges like a coin
add_coin_reeds=true;
//EDGE: adds_edge notch colors
add_edge_notches=true;
//TOP: adds the ring ot 13 stars around the top rim
...
back_inner_arch_top_text="POST  7116";
//bottom arched text on back
back_inner_arch_bottom_text="SYLVANIA GA";
...
module poker_chip() 
{
    diff() 
    {
        ...
        cyl(d=chip_diameter, h=chip_thickness, rounding=edge_rounding, center=true) 
        {
            ...
            {    
                tag("remove") wrapped_text_logo_bottom(back_inner_arch_top_text, back_inner_arch_bottom_text); 
            }    
...
{
    // Millimeter size you want the image to occupy on the chip face
    bottom_logo_target_width = 20; 
    bottom_logo_target_length = 9;
    ...
    { 
        ...
        {  
            back(0.5)
            resize([bottom_logo_target_width, bottom_logo_target_length])
                offset(r=-0.01) 
                    offset(r=0.01)
...
module builder()
{
   if (render=="all" || render=="body")
    ...
    if (render == "all" || render=="bottomtext")
    {
        ...
        if (add_wrapped_text_logo_bottom==true)
        {    
            color("white") wrapped_text_logo_bottom(back_inner_arch_top_text, back_inner_arch_bottom_text);
        }    
        if(add_outer_text_logo_bottom==true)
        {    
