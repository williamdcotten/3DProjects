...
module poker_chip() 
{
    diff() 
    {
        ...
        cyl(d=chip_diameter, h=chip_thickness, rounding=edge_rounding, center=true) 
        {
            ...
            {
                tag("remove") inner_text_logo_top();
            }    
            if (add_inner_text_logo_bottom==true)
            {
                tag("remove") inner_text_logo_bottom();
            }  
            if (add_wrapped_logo_top)
            {
                tag("remove") wrapped_text_logo_top();
            }    
...
module inner_text_logo_top() 
{
    ...
    {
        back(2) //center is 3
            text("20", size=10, font="Liberation Sans:style=Bold", 
                 halign="center", valign="center");
        
        fwd(5)//originally 4 
            text("DAYS", size=3, font="Liberation Sans:style=Bold", 
                 halign="center", valign="center");
    }
}
...
module inner_text_logo_bottom() 
{
    ...
    {
        back(8) 
            text("I'VE", size=6, font="Liberation Sans:style=Bold", 
                 halign="center", valign="center");
        
        text("GOT", size=6, font="Liberation Sans:style=Bold", 
                 halign="center", valign="center");
        
        fwd(8) 
            text("THIS", size=6, font="Liberation Sans:style=Bold", 
                 halign="center", valign="center");
    }
}
...
module wrapped_text_logo_top() 
{
    ...
    font_safety_buffer = 2.6;            // Distance away from the rim edge
    ...
    down(recess_depth) 
    {
        ...
        linear_extrude(height=recess_depth) 
        {
            ...
            for (i = [0 : len(top_text)-1]) {
                angle = (i - (len(top_text)-1)/2) * 13; //10.5; 
                rotate([0, 0, -angle + 90]) 
        ...
        linear_extrude(height=recess_depth) 
        {
            ...
            {
                angle = (i - (len(bot_text)-1)/2) * 13;//9.5; 
                rotate([0, 0, angle - 90]) 
...
{    
    ...
    if (add_wrapped_logo_top==true)
    {    
        color("white") wrapped_text_logo_top();
    }
    if(add_outer_text_logo_top==true)
    {    
        color("white") outer_text_logo_top();
    }
 } 
